 ...
 *   2. Once joined, inference runs every INFERENCE_INTERVAL_MS (10 s).
 *   3. Each result is packed into a 14-byte payload and transmitted
 *      immediately, provided the radio stack is idle.
 *   4. If a TX is already in flight when inference finishes, the new
...
// ── Edge Impulse ──────────────────────────────────────────────────────────────
#include <Master_V2_inferencing.h>
...
// CONFIGURATION
// =============================================================================
#define INFERENCE_INTERVAL_MS  10000UL   // classify audio every 10 seconds
...
// =============================================================================
static const char* CATEGORY_LABELS[5] = {
    "ambient_weather",   // 0
    "traffic_transport", // 1
    "animals",           // 2
    "human_activity",    // 3
...
static uint32_t last_inference_ms = 0;  // millis() timestamp of last inference
...
void setup() {
    ...
    Serial.print(F("[INF] Classify every ")); Serial.print(INFERENCE_INTERVAL_MS / 1000); Serial.println(F(" s"));
...
// =============================================================================
// loop()  –  drives LMIC; runs inference every INFERENCE_INTERVAL_MS after join
// =============================================================================
void loop() {
    ...
    // Check whether it is time for another inference cycle
    uint32_t now = millis();
    if (now - last_inference_ms < INFERENCE_INTERVAL_MS) return;
    last_inference_ms = now;

    // ── Run inference and update payload buffer ───────────────────────────────
    Serial.println(F("\n[INF] === Starting inference cycle ==="));
